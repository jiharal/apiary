...
import (
	...
	"github.com/jiharal/apiary/system"
...
func (m UserModule) Detail(ctx context.Context, id int64) (model.UserModelResponse, *Error) {
...
func (m UserModule) Update(ctx context.Context, param UserDataParam) (model.UserModelResponse, *Error) {
	hash, err := HashPassword(ctx, param.Password)
	if err != nil {
		return model.UserModelResponse{}, NewErrorWrap(err, m.name, "create/hash",
			MessageGeneralError, http.StatusInternalServerError)
	...
	if param.Name != "" || param.Email != "" || param.Password != "" {
		...
		user.Password = hash
		user.UpdatedBy = param.By
	}

	_, err = user.Update(ctx, m.db)
	if err != nil {
		return model.UserModelResponse{}, NewErrorWrap(err, m.name, "update",
			MessageGeneralError, http.StatusInternalServerError)
	}
...
func (m UserModule) Delete(ctx context.Context, id int64) (interface{}, *Error) {
